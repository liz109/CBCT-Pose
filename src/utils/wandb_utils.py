...
def initialize(args, exp_name, project_name):
    ...
    # key = '4b617b1e2138454ab54f904395e95d904fb35dc6'
    # wandb.login(key=key)
...
def log_image(sample, step=None):
    if is_main_process():
        sample = array2grid(sample) # np/pil image (h, w, c)
...
def array2grid(x):
    ...
    nrow = round(math.sqrt(len(x)))
    x = make_grid(x, nrow=nrow, normalize=True, value_range=(0,1))
    x = x.mul(255).clamp_(0,255).to('cpu', torch.uint8).numpy()
    return x