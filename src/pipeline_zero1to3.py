...
class Zero1to3StableDiffusionPipeline(DiffusionPipeline, StableDiffusionMixin):
    ...
    def __init__(
        ...
        self.clip_img_processor = AutoProcessor.from_pretrained(
    ...
    def decode_latents(self, latents):
        ...
        # ref line 274: https://github.com/huggingface/diffusers/blob/main/examples/community/clip_guided_stable_diffusion_img2img.py
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None):
        ...
        if latents is None:
            ...
            # latents = torch.zeros(shape).to(device)
        else:
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            # 8. Post-processing
            image = self.decode_latents(latents)    # image in [0, 1]
            # 10. Convert to PIL
            image = self.numpy_to_pil(image)
        else:
            # 8. Post-processing
            image = self.decode_latents(latents)    # image in [0, 1] with (h, w, 3)
