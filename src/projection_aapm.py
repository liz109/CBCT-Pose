...
class ProjectionData(Dataset):
    ...
    def __init__(self, path, type="train", size=256, coords=None) -> None:
        ...
        image_transforms.extend([
            # transforms.Lambda(lambda x: rearrange(x * 2. - 1., 'h w c -> c h w')),
            transforms.ToTensor(),      # -> c h w
            transforms.Lambda(lambda x: x * 2. - 1.),   # x [0,1] -> [-1, s1]
        ...
        if type == "train":
            self.projs = data["projections"]   # (100, 1024, 1024)
            self.angles = data["angles"]       # (100,)
            self.n_projs = data["numTrain"]             # 100
        ...
        # elif type == "val":
        #     self.projs = data["projections"] 
        #     self.angles = data["angles"]        
        #     self.n_projs = len(self.angles)
        #     coords = torch.stack(torch.meshgrid(
        #         torch.linspace(1, self.n_projs-1, self.n_projs//2), 
        #         torch.linspace(0, self.n_projs-2, self.n_projs//2), indexing="ij"), -1)  
        #     self.coords = torch.reshape(coords, [-1, 2])    # [-1, 2 ij]: (target, cond)

        elif type == "sample":
            self.projs = data["projections"]     # (50, 256, 256)
            self.angles = data["angles"]         # (50,)
            self.n_projs = len(self.angles)
            self.coords = coords
    ...
    def __getitem__(self, index):
        ...
        data["target_image"] = target_im    # (h, w, 3)
        data["cond_image"] = cond_im
    ...
    def process_im(self, im):
        x = im[..., np.newaxis]
        x = x.repeat(3, axis=-1)
        return self.image_transforms(x)
